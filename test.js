...
describe('highlightHTMLContent function', () => {
  it('should highlight the content in HTML based on plain text positions', () => {
    ...
    const plainText = "Energix Closes $520 Million Financing and Tax Equity Deal";
    ...
    const receivedOutput = highlightHTMLContent(htmlContent, plainText, plainTextPositions);
    const normalizedExpectedOutput = prettier.format(expectedOutput, { parser: 'html' });
    const normalizedReceivedOutput = prettier.format(receivedOutput, { parser: 'html' });

    expect(normalizedReceivedOutput).toEqual(normalizedExpectedOutput);
  });
